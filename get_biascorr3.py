...
#from numpy import *
## change 9/17/19 to test; need dateutils from current directory

import math, sys, os,datetime, string, time
import dateutils3
from numpy import *
...
#    basin = int(str(ncode)[-2])
  # oct 20 17 lcs: make all TC errors 2.5
#    oberrdict[ncode]=2.5
...
## 25 Oct 2017 make all=2.5, L. Slivinski
#for ncode in range(500,599,10):
     #oberrdict[ncode]=2.5
     
# 1 Nov 2017 make all TCs = 2.5 G. Compo
for ncode in range(300,499):
     oberrdict[ncode]=2.5   
for ncode in range(500,599):
     oberrdict[ncode]=2.5   
#    basin = int(str(ncode)[-2])
...
fileout = open(obsfileout,'w', encoding="ISO-8859-1")


# added by gpc
# get day of year, datetime instance corresponding to analysis time
yyyy,mm,dd,hh = dateutils3.splitdate(analdate)
...
analdate2 = dateutils3.dateshift(analdate,-6)
analdate1 = dateutils3.dateshift(analdate2,-lastndays*24)
dates = dateutils3.daterange(analdate1,analdate2,6)
ndays = len(dates)/4 
obdict = {}
# SEG 22Aug24 removed 
#  if wanted in future (Python3)
#   t1=time.process_time()
#t1 = time.clock()
for date in dates:
    pathobs = os.path.join(datapath,date)
    try:
        file_ob = open(os.path.join(pathobs,'psobs_prior.txt'), encoding="ISO-8859-1")
    except:
        continue
    for line in file_ob:
#       skip first 64 chars in line (contains ob identification string)
    ...
    #   dateob = dateutils3.dateshift(date,int(float(timeob)))
        key = " %07s%06s %05s %03s" % (lonob,latob,zob,obtyp)
        if key in obdict:# obdict.has_key(key):
            val = obdict[key]
...
for line in open(os.path.join(pathobs,'psobfile'), encoding="ISO-8859-1"):
#1999103121000000013 181 P   39.30  44.30   323   0.00   985.50  999.40  1.20   -1.22
    ...
    try:
        yyyy,mm,dd,hh = dateutils3.splitdate(dateob)
    except ValueError:
        continue
    try:
...
# skip stations with huge errors.
    ...
    key = " %07s%06s %05s %03s" % (lonob,latob,zob,obtyp)
    if key in biasdict:#biasdict.has_key(key):
        bias = biasdict[key]
    else:
        bias = 1.e30
    # no bias correction.
