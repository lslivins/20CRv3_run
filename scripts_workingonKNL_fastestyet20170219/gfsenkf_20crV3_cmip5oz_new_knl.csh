...
setenv NNODES_sml 4 
setenv NNODES_vsml 24

 setenv corespernode 65 #65 # can be up to 68. Will need to experiment with best setting
 setenv maxcorespernode 64 # on KNL the cores per node and maxcoresper node are set separately
 setenv maxnhosts `expr $NNODES \* $maxcorespernode`#5200 # should be able to do as expression, but didn't work? 
 setenv corespernumanode 32 #65 # not sure what this should be, but I don't think it is being used.
...
setenv fg_omp_threads 2#2
setenv coresperfgmpitask 4
...
setenv enkf_taskspernode 32#64 # will need to change explicitly in run_enkf_ps_h5_biascor_iau_knl
...
echo "$SLURM_JOB_ID LF `date +%Y%m%d%H%M` ncycles=$ncycles $analdate $walltime $NNODES $nhosts $corespernode $corespermpitask $enkf_taskspernode $OMP_NUM_THREADS $fg_omp_threads" >> ${enkfscripts}/walltime.out
#endif
