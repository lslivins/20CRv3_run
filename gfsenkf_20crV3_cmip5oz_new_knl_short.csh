...
#SBATCH --job-name=1904_451_gfsenkf
#SBATCH --account=m958
#SBATCH --qos=shared_overrun
#SBATCH --constraint=cpu
#### nodes limited by queue
##SBATCH -N 80    
#SBATCH -N 1
#SBATCH --ntasks-per-node 32 
#SBATCH -t 00:46:00
#SBATCH --time-min=00:36:00
...
# updated python codes rewrite2.py, get_biascorr3.py and dateutils3.py for python3 from L. Slivinski (8 Jan 2020, G. Compo)
...
set path = ($path /pscratch/sd/s/segeorge/20CRV3_CoriII_stripe4/bin/ )
...
set timemax=1 # this is in hours. Make timemax larger than you want to run for. 
...
setenv corespermpitask 8 # number of cores used by EnKF MPI task and available for OpenMP thread 
		#and other: 
		### SEG setenv corespermpitaskd5 52# number of cores per mpi task to use for NNODESd5 on knl 
setenv corespermpitaskd5 32# number of cores per mpi task to use for NNODESd5 on knl 
...
setenv basedir /pscratch/sd/s/segeorge/20CRV3_CoriII_stripe4/
...
##SEG set ensda = "run_enkf_ps_h5_biascor_iau_knl"
set ensda = "run_enkf_ps_h5_biascor_iau_knl_cut"
set fg_gfs = "run_fg_gfs_ens_iau_knl_cut" # calls run_gefs_knl
...
set datadir=/pscratch/sd/s/$USER/
...
setenv obsdirh5 /pscratch/sd/c/cmccoll/HDF5/V47_china
...
setenv sstpath /pscratch/sd/s/segeorge/gfsenkf_20crV3_cmip5oz_CoriII/ensda_461_1924
...
if ($analdate >= `$incdate $startdate 12`) then
 time python3 ${enkfscripts}/get_biascorr3.py ${analdate} ${datapath2}/psobs.txt  &
else
 # to skip bias correction, use this...
 time python3 ${enkfscripts}/rewrite2.py ${datapath2}/psobfile ${datapath2}/psobs.txt $analdate &
endif
...
set segcpu = 16  # Node 2 chips, 64 cores x2 thread each
### CPU = 2 * (128/tasks per node)
# SEG Set tasks-per-node in header 
if ($POSTPROC == "YES") then
  echo "compute ens mean panl file"
  ##SEG time srun  -N $NNODESd5 -n $nanals -c $corespermpitaskd5 --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdatem1 p anl  &
  #time srun   -n $nanals -c $segcpu --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdatem1 p anl  &
  time srun   --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdatem1 p anl  &
  #time srun   --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdatem1 p anl  &
  echo "compute ens mean panl+3 file"
  ## SEG time srun  -N $NNODESd5 -n $nanals -c $corespermpitaskd5 --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdate_p3 p anl  &
  ##time srun  -n $nanals -c $segcpu --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdate_p3 p anl  &
  time srun  --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdate_p3 p anl  &
  echo "compute ens mean pfg file"
  ##SEG time srun  -N $NNODESd5 -n $nanals -c $corespermpitaskd5 --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $IO $JO $analdate p fg 06 &
  ##time srun  -n $nanals -c $segcpu --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $IO $JO $analdate p fg 06 &
  time srun  --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $IO $JO $analdate p fg 06 &
  echo "compute ens mean sflxf03 file"
  ##SEG time srun  -N $NNODESd5 -n $nanals -c $corespermpitaskd5 --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 03 & 
  ##time srun  -n $nanals -c $segcpu --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 03 & 
  time srun   --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 03 & 
  echo "compute ens mean sflxf06 file"
  ##SEG time srun  -N $NNODESd5 -n $nanals -c $corespermpitaskd5 --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 06 & 
  ##time srun  -n $nanals -c $segcpu --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 06 & 
  time srun  --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 06 & 
endif
wait
