...
setenv enkf_taskspernode $corespernode # will need to change explicitly in run_enkf_ps_h5_biascor_iau_knl
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/bfg_${analdate}_${charfhr}_ensmean)) then
     ...
     time srun -n $nanals  -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/getsfcensmeanp.x ${datapath}/${analdate}/ bfg_${analdate}_${charfhr}_ensmean bfg_${analdate}_${charfhr} ${nanals}  &
  endif
  @ fh = $fh + $FHOUT
end
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/sfg_${analdate}_${charfhr}_ensmean)) then
   ...
   time srun -n $nanals  -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/getsigensmeanp.x ${datapath}/${analdate}/ sfg_${analdate}_${charfhr}_ensmean sfg_${analdate}_${charfhr} ${nanals} & 
  endif
  #@ fh = $fh + $FHOUT
  @ fh = $fh + $FHOUT
end
...
if ($POSTPROC == "YES") then
  ...
  time srun -n $nanals  -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdatem1 p anl  &
  echo "compute ens mean panl+3 file"
  time srun -n $nanals  -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdate_p3 p anl  &
  echo "compute ens mean pfg file"
  time srun -n $nanals  -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $IO $JO $analdate p fg 06 &
  echo "compute ens mean sflxf03 file"
  time srun -n $nanals  -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 03 & 
  echo "compute ens mean sflxf06 file"
  time srun -n $nanals  -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 06 & 
endif
wait
...
echo "`date +%Y%m%d%H%M` ncycles=$ncycles $analdate $walltime $nhosts $corespernode $corespermpitask $OMP_NUM_THREADS $fg_omp_threads" >> ${enkfscripts}/walltime.out
#endif
